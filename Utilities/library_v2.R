...
Est.ALASSO.GLM = function(data,Wi=NULL,rtn="EST",nopen.ind=NULL,regularize=yes.regularize,pwr=0.1){
  ...
  if(regularize){
    ...
    lam.ridge = 0.001; ##bini = plr(x,y,lambda=lam.ridge,weights=Wi)$coef; 
...
ROC.Est.FUN <- function(Di,yyi,yy0,fpr0=NULL,wgti=NULL,yes.smooth=F)
  {
    #TPR: true positive rate - SENSITIVITY=RECALL; PPV: positive predictive value - PRECISION
    #F-score: 2/(1/TPR + 1/PPV) - harmonic mean of TPR and PPV
