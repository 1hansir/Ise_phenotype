...
def get_data_from_csv(dirr_data, filename, filename_RP,dirr_save,colums_min,colums_max,visit_maximum,file_risk_factors,
    ...
    Time_index_smooth = np.zeros(len(data_Time))         # 连续的时间序列
    ...
    for rowi in range(1, len(data_Time)):
        ...
        '''if str(Patient_num[rowi]) in dic_patient_label and str(Patient_num[rowi]) == str(Patient_num[rowi-1]):  # 消除重复样本的影响
            dic_patient_label[str(Patient_num[rowi])].append(int(Y_label[rowi]))
            Time_index_smooth[rowi] = t
            t = t + 1
        else:
            dic_patient_label[str(Patient_num[rowi])] = [int(Y_label[rowi])]
            t = 1'''
        # t 为按顺序排布的时间(以应对有缺失的时间点)
        # 为什么要让这个变成最终的label? 应该是每一个位置的label才对吧！
        dic_patient_label[str(Patient_num[rowi])] = int(Y_label[rowi])
    ...
    #print(dic_patient_label[str(Patient_num[0])])
    #print(Time_index_smooth[0:30])
    key_feature1 = list(df.columns[1])         ###if need key features
    key_feature2 = list(df.columns[2])         ###if need key features
    #dic_patient_label = np.array(dic_patient_label)
        ...
        if (int(patient) in patient_targeted) and str(patient) in dic_map_results :
            if (int(patient) in patient_targeted or 'ALL' in patient_targeted) and str(patient) in dic_map_results :  ##having data after observation period,this is unnecessary
                ...
                if patient in patients_total and patient in dic_patient_firstDate and patient in dic_patient_lastDate:
                    ...
                    if int(data_Time[rowi]) < dic_patient_firstDate[patient] + Observation_months:
                        ...
                        patients_total[patient][index_row, 1] = int(data_Time[rowi] - (dic_patient_firstDate[patient]))
                        patients_total[patient][index_row, 0] = int(Patient_num[rowi])

                        #############
                        patients_total_RP[patient][index_row, 5] = 1
                        patients_total_RP[patient][index_row, 3] = dic_patient_lastDate[patient] - \
                                                                   dic_patient_firstDate[patient]
                        #############the MAP results as the keyfeature2: using the last 20 windows MAP results
                        patients_total_RP[patient][index_row, 4] = np.mean(
                            dic_map_results[patient][-1:])  ###the last one?
                        # patients_total[patient][index_row, 4] = np.mean
                        ...
                        patients_total_RP[patient][index_row, 1] = int(
                            data_Time[rowi] - (dic_patient_firstDate[patient]))
                        ...
                        ###### data after the observation: to provide labels for different censor times
                        ...
                        patients_total_RP[patient][
                            index_row, 5] = 0  #### set to be zero since future risk factors cannot be observed
                        patients_total_RP[patient][index_row, 3] = dic_patient_lastDate[patient] - \
                                                                   dic_patient_firstDate[
                                                                       patient]
                        #############the MAP results as the keyfeature2: using the last 20 windows MAP results
    ...
    dir_ = dirr_save + "PKL_files/" + str(filename_save).split(".csv")[0]
    if not os.path.exists(dir_):
        os.makedirs(dir_)  # 创建多级目录

    with open(dir_ + str(filename_save).split(".csv")[1] , 'wb') as fid:
